    ...
    <header className="sticky top-0 z-50 border-b border-black/10 bg-signal text-white">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 md:px-8 py-3">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          ...
          <Link href="/contact" className="inline-flex items-center rounded-md bg-alloy text-navy px-3 py-1.5 hover:bg-alloy/90">Contact</Link>
        </nav>
        <button onClick={()=>setOpen(true)} aria-label="Open menu" className="md:hidden inline-flex items-center justify-center rounded-md border border-white/20 px-3 py-2">
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black text-white">
          <div className="mx-auto flex max-w-6xl items-start justify-between px-6 md:px-8 py-4">
            <Link href="/" onClick={()=>setOpen(false)} className="font-mokoto">WILLRISE</Link>
            <button onClick={()=>setOpen(false)} className="inline-flex items-center justify-center rounded-lg border border-white/40 p-2" aria-label="Close menu">
          ...
          <div className="mx-auto flex min-h-[calc(100vh-120px)] max-w-md flex-col items-stretch px-6 md:px-0">
            <div className="mx-auto w-full space-y-6 text-center">
              ...
              <div className="pt-2">
                <Link href="/contact" onClick={()=>setOpen(false)} className="inline-block rounded-lg bg-alloy text-navy px-6 py-3 font-bicubik tracking-[0.2em] hover:bg-alloy/90">
                  CONTACT
                </Link>
              </div>
            </div>

    ...
    <section className="py-14 md:py-24 bg-gradient-to-b from-white to-neutral-50 dark:from-black dark:to-neutral-950">
      <div className="mx-auto grid max-w-6xl items-center gap-10 px-6 md:px-8 md:grid-cols-2">
        <div className="space-y-6">
          <h1 className="font-mokoto text-[clamp(2.2rem,2.8vw+1rem,3.6rem)] leading-[1.05]">
            {site.hero.title}
          </h1>
          ...
          <div className="flex items-center gap-3 pt-2 text-xs opacity-80">
            {site.hero.badges.map((b) => (
              <span key={b} className="rounded bg-neutral-100 px-2 py-1 dark:bg-neutral-800">{b}</span>
            ))}
          </div>
        </div>
  ...
  return (
    <>
      ...
      <Section id="intro-video">
        ...
        {/* Explainer text card (no 'The Problem' heading) */}
        <div className="mx-auto mt-8 max-w-4xl rounded-2xl border border-alloy/30 bg-gradient-to-b from-neutral-50 to-white p-6 text-center shadow-sm dark:from-neutral-900 dark:to-neutral-950">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <div className="rounded-lg border border-alloy/40 p-3">
              <div className="text-xl font-semibold">3–5 min</div>
              <div className="text-xs opacity-75">Time to loss of consciousness in adverse suspension scenarios.</div>
            </div>
            <div className="rounded-lg border border-alloy/40 p-3">
              <div className="text-xl font-semibold">8–15 min</div>
              <div className="text-xs opacity-75">Window from blackout to fatal outcome without rescue.</div>
            </div>
            <div className="rounded-lg border border-alloy/40 p-3">
              <div className="text-xl font-semibold">0 tools</div>
              <div className="text-xs opacity-75">Most wearers lack practical self-relief under real-world load.</div>
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section id="verticals" title="Applications" center>
        <div className="grid gap-6 md:grid-cols-3">
          {[
            ...
            <a key={href as string} href={href as string} className="rounded-xl border border-alloy/40 p-0 hover:border-signal overflow-hidden">
              <div className="aspect-[4/3] w-full bg-neutral-100 dark:bg-neutral-900">
                <img src={img as string} alt={`${t as string} placeholder`} className="h-full w-full object-cover" />
              </div>
              <div className="p-5">
                <div className="font-medium">{t}</div>
...
<Section id="blog" title="Latest from the Blog" center>
  <div className="grid gap-6 md:grid-cols-3">
    {posts.slice(0,3).map(p => (
      <a key={p.slug} href={`/blog/${p.slug}`} className="rounded-xl border border-alloy/40 p-5 hover:border-signal">
      ...
      <Section id="contact" title="Contact Us" center>
        <LeadForm />
      </Section>
    </>
  );
